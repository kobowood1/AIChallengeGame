...
class Admin(UserMixin, db.Model):
    """Model for administrator authentication and management"""
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(80), unique=True, nullable=False)
    email = db.Column(db.String(120), unique=True, nullable=False)
    password_hash = db.Column(db.String(256), nullable=False)
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    last_login = db.Column(db.DateTime, nullable=True)
    created_by = db.Column(db.Integer, db.ForeignKey('admin.id'), nullable=True)  # Which admin created this admin
    ...
    def can_download_reports(self):
        """Check if admin can download reports"""
        return self.role in ['admin', 'super_admin']
