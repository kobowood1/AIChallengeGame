...
from sendgrid.helpers.mail import (Mail, Attachment, FileContent, FileName,
                                   FileType, Disposition, ContentId, Email, To,
                                   Content, MailSettings, SandBoxMode)
...
FROM_EMAIL = "multicyberboy@gmail.com"  # Replace this with your actual verified sender email

# Recipients for reflection reports
RECIPIENTS = ['aturan@asu.edu', 'kobowood1@gmail.com', 'JANEL.WHITE@asu.edu']
...
def send_reflection_report(participant_info, report_content, participant_id):
    ...
    if not SENDGRID_API_KEY:
        ...
        logger.warning(
            "Email requires a valid SENDGRID_API_KEY environment variable.")
        return False
    ...
    if participant_info and participant_info.get('occupation'):
        occupation = participant_info.get('occupation',
                                          '').replace(' ', '_').lower()
    ...
    try:
        ...
        for recipient in RECIPIENTS:
            ...
            if status_code in [200, 202]:
                logger.info(
                    f"Email sent successfully to {recipient} with status code {status_code}"
                )
